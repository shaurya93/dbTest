...
app.get('/login', function(req,res,next){
	...
	var query9 = "ALTER TABLE vendor_master ALTER COLUMN phone DROP NOT NULL,ALTER COLUMN address DROP NOT NULL";
	var f9 = {query:query9};
	dbs.query(null,f9);
	dbs.update();
	
	*/
	var query71 = "Alter table vendor_master add column delivery_method varchar(10),add column contact_name varchar(50),add column isperson varchar(20),add column contact_phone varchar(20)";
...
if(id)
{
	...
	var lastname_val = null;
		...
		{
			console.log("Phone -->" + req.query.contact_number);
			phone_val = req.query.contact_number;
		}
		if(req.query.address-line1||req.query.address-line2||req.query.city||req.query.region||req.query.postal-code||req.query.country)
		{
			console.log("Address --> " + req.query.address-line1 + "\n" + req.query.address-line2 +"\n" + req.query.city + "\n" + req.query.region + "\n" + req.query.country + "\n" + req.query.postal-code );
			address_val = req.query.address-line1 + "\n" + req.query.address-line2 +"\n" + req.query.city + "\n" + req.query.region + "\n" + req.query.country + "\n" + req.query.postal-code;
		}
		console.log("Password -->" + req.query.password);
		password_val = req.query.password;
		if(req.query.alternate_contact_number)
		{
			console.log("Alt-Phone -->" + req.query.alternate_contact_number);
			altphone_val = req.query.alternate_contact_number;
		}
		if(req.query.faxno)
		{
			console.log("Fax -->" + req.query.faxno);
			fax_val = req.query.faxno;
		}
		if(req.body.first_name||req.body.last_name)
		{
			...
			var rest_params = {event:"refresh",shipment_origin:shipment_origin_val,shipment_point:shipment_point_val,delivery_method:delivery_method_val,currency:currency_val,companyname:company_name_val,delivery_lead_time:delivery_lead_time_val,exfactory_lead_time:exfactory_lead_time_val,terms:terms_val,incoterm:incoterm_val,security_question:security_question_val,security_answer:security_answer_val,email:email_val,phone:phone_val,address:address_val,password:password_val,altphone:altphone_val,fax:fax_val,firstname:firstname_val,lastname:lastname_val,isperson:"Individual"};
			
		}
		else
		{
			//console.log("Contact Email -->" + req.query.contact_email);
			//var contact_email_val = req.query.contact_email;
			
			if(req.query.contact_person)
			{
				console.log("Contact Name -->" + req.query.contact_person);
				contact_name_val = req.query.contact_person;
			}
			if(req.query.con_number)
			{
				console.log("Contact Phone -->" + req.query.con_number);
				contact_phone_val = req.query.con_number;
			}
			
			var rest_params = {event:"refresh",shipment_origin:shipment_origin_val,shipment_point:shipment_point_val,delivery_method:delivery_method_val,currency:currency_val,companyname:company_name_val,delivery_lead_time:delivery_lead_time_val,exfactory_lead_time:exfactory_lead_time_val,terms:terms_val,incoterm:incoterm_val,security_question:security_question_val,security_answer:security_answer_val,email:email_val,phone:phone_val,address:address_val,password:password_val,altphone:altphone_val,fax:fax_val,contact_person_name:contact_name_val,contact_phone:contact_phone_val,isperson:"Company"};	
		}
...
app.get('/refreshitemreceipts',function(req, res, next) {
		...
		else
		{
			...
			for(var j=0;j<results.itemreceiptdetails[i].itempush.length;j++)
			{
				...
				*/
				...
				var is_exist=isItemInArray(item_receipt_list,[results.itemreceiptdetails[i].tranID,results.itemreceiptdetails[i].itempush[j].item_id]);
				...
				else
				{
					var update_query = "update item_receipt_lines set item_receipt_id='"+results.itemreceiptdetails[i].ns_id
								...
								+ "',DESCRIPTION='" + results.itemreceiptdetails[i].itempush[j].itemName
...
// trigger the restlet
		...
		// insert or update PO and PO lines
			 for(var i=0;i<results.itemreceiptdetails.length;i++)
			{
				...
				var index=item_receipt_list.indexOf(parseInt(results.itemreceiptdetails[i].tranID));
				console.log('index::'+index);
...
{
	setTimeout(function () {
           res.redirect(303,'/ItemReceiptsView?ns_id='+item_receipt_ns_id);
        }, 5000);
...
if(id)
{
		...
		*/
		...
		dbs.readData(function (result1){
			...
			created_date_val = result1[i].created_date; 
...
app.get('/d3', function(req, res, next) { // route for '/'
	...
	/*
	 	var query_json_PO = "SELECT row_to_json(PO) AS PURCHASE_ORDER FROM (select * from purchase_order where vendor_ns_id::integer="+id+") PO";
	var fields_query_json_PO = {query:query_json_PO};		
	dbs.query(null,fields_query_json_PO);
	dbs.readDirectData(function (result){
		json_PO = JSON.stringify(result);
		
		console.log("json_PO :: "+ json_PO);
		});
	var query_json_PackingList = "SELECT row_to_json(PL) AS PACKING_LIST FROM (select * from PACKING_LIST where po_ns_id='5172') PL";
	var fields_query_json_PackingList = {query:query_json_PackingList};		
	dbs.query(null,fields_query_json_PackingList);
	dbs.readDirectData(function (result){
		json_PackingList = JSON.stringify(result);
		console.log("json_PO :: "+ json_PackingList);
		});
	
	
	*/	
	
	//var query_json_PackingList = "SELECT row_to_json(PL) AS PACKING_LIST FROM (select 'Packing List' as name, CONCAT('Packing List::',packing_list_ns_id) as assigned,date_created as Date ,status from PACKING_LIST where po_ns_id='5172') PL";
	//var query_json_PackingList = "select 'Packing List' as name, CONCAT('Packing List::',packing_list_ns_id) as assigned,date_created as Date ,status from PACKING_LIST where po_ns_id='5172'";
	//var query_json_PackingList = "select * from PACKING_LIST where po_ns_id='5172'";
	//var fields_query_json_PackingList = {query:query_json_PackingList};		
	//dbs.query(null,fields_query_json_PackingList);
	//dbs.readDirectData(function (result){
	...
	dbs.readData(function (result){
		...
		for (var i=0; i<result.length; i++) 
		{
			...
			//plChild[i] = {"name":"Packing List","assigned":"Packing List::"pl_id[i],"date":pl_date[i],"status":"Shipped"}}
			
			plChild.push({"name":"Packing List","assigned":"Packing List::"+pl_id[i],"date":pl_date[i],"status":pl_status[i]});
		...
		console.log("plChild : "+plChild);
		console.log("plChild : "+JSON.stringify(plChild));
		
		poParent.push({"name":"Purchase Order","assigned":"5172","children":JSON.stringify(plChild[0])});
		console.log("poParent : "+JSON.stringify(poParent[0]));
		poParent = json_encode(poParent,JSON_UNESCAPED_SLASHES);
