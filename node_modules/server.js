...
if(req.query.vendorid)
{
	...
	*/
    ...
    contents_po_all_count.on("end", function(result) {
      ...
      //	console.log('Length :: ' + result.rowCount);
	/*	for(var i=0;i<result.rowCount;i++ )
		{
			console.log('PO :: '+ i + '::' +result[i].po_number);
		}*/
    });

console.log('po_all-count::'+po_all_count);
/*	contents_po_all_count.on("row", function(row,result)
        {
        	result.addRow(row);
             	contents_po_all_count = result.rows;
             	
             	console.log('Length :: ' + contents_po_all_count.length);
		for(var i=0;i<contents_po_all_count.length;i++ )
		{
			console.log('PO :: '+ i + '::' +contents_po_all_count[i].po_number);
		}
             	
             	//console.log('contents in po_all_count: ' + JSON.stringify(result.rows));
             	console.log('result ends here');
        });
*/	
	...
	{
		po_all_count = contents_po_all_count.rowCount;
	}
	*/
	...
	contents_po_pendingAck_count.on("row", function(row,result)
        {
        	result.addRow(row);
             	contents_po_pendingAck_count = result.rows;
        });
	
	
	if(contents_po_pendingAck_count!='')
	{
		po_pendingAck_count = contents_po_pendingAck_count.rowCount;
	}
	...
	contents_po_Ack_count.on("row", function(row,result)
        {
        	result.addRow(row);
             	contents_po_Ack_count = result.rows;
        });
	
	if(contents_po_Ack_count!='')
	{
		po_Ack_count = contents_po_Ack_count.rowCount;
	}
	...
	contents_po_pendingDelivery_count.on("row", function(row,result)
        {
        	result.addRow(row);
             	contents_po_pendingDelivery_count = result.rows;
        });
	
	if(contents_po_pendingDelivery_count!='')
	{
		po_pendingDelivery_count = contents_po_pendingDelivery_count.rowCount;
	}
	...
	contents_po_pendingBill_count.on("row", function(row,result)
        {
        	result.addRow(row);
             	contents_po_pendingBill_count = result.rows;
        });
	
	if(contents_po_pendingBill_count!='')
	{
		po_pendingBill_count = contents_po_pendingBill_count.rowCount;
	}
	...
	contents_po_fully_billed_count.on("row", function(row,result)
        {
        	result.addRow(row);
             	contents_po_fully_billed_count = result.rows;
        });
		
	if(contents_po_fully_billed_count!='')		
	{		
		po_fully_billed_count = contents_po_fully_billed_count.rowCount;		
	}		
	...
	contents_pl_readyToShip_count.on("row", function(row,result)
        {
        	result.addRow(row);
             	contents_pl_readyToShip_count = result.rows;
        });

	if(contents_pl_readyToShip_count!='')
	{
		pl_readyToShip_count = contents_pl_readyToShip_count.rowCount;
	}
	...
	contents_pl_shipped_count.on("row", function(row,result)
        {
        	result.addRow(row);
             	contents_pl_shipped_count = result.rows;
        });
	
	if(contents_pl_shipped_count!='')
	{
		pl_shipped_count = contents_pl_shipped_count.rowCount;
	}
	
	//var contents_bill_all_count = db.exec("select * from bill_list");
	var contents_bill_all_count = client.query("select * from bill_list");
	
	contents_bill_all_count.on("row", function(row,result)
        {
        	result.addRow(row);
             	contents_bill_all_count = result.rows;
        });
	
	if(contents_bill_all_count!='')
	{
		bill_all_count = contents_bill_all_count.rowCount;
	}
	*/
...
app.get('/login', function(req,res,next){
	...
	var query1 = client.query("CREATE TABLE timestamp"+
		...
		query5.on("end", function (result) {          
	//			client.end(); 
	//			res.write('Table bill list lines Created');
				console.log('Table bill list lines Created');
	//			res.end();  
			});
