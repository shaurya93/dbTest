					 ...
					 for(var i=0;i<results.billdetails.length;i++)
					{
						...
						else
						{
							...
							for(var j=0;j<results.billdetails[i].itempush.length;j++)
							{
								...
								{
									var po_Number = 0;
									console.log("Gross Wt if null :: " + po_Number);
								}
								else 
								{
									po_Number = results.billdetails[i].itempush[j].poNumber;
								}
		...
		else
		{
			...
			for(var j=0;j<results.billdetails[i].itempush.length;j++)
			{
				...
				if(results.billdetails[i].itempush[j].taxamount!="")
						...
						if(results.billdetails[i].itempush[j].poNumber!=null)
						{
							var po_Number = parseInt(results.billdetails[i].itempush[j].poNumber);
							console.log("PO Number if not null :: " + po_Number);
						}
						else
						{
							var po_Number = 0;
							console.log("PO Number if null :: " + po_Number);
						}
						var total_amount = parseInt(results.billdetails[i].itempush[j].amount*neg_factor_amt) + tax_amount ;		
				if(!is_exist)
				{
...
//var timestamp_contents = db.exec("select localtime from timestamp where rec_type= 'Bills'");
	...
	if(result != null)
	{
		...
		for (var i=0; i<result.length; i++) 
		{
			...
			date_create[i] = month[date[i].getMonth()] +"  "+ date[i].getDate() +"  "+ date[i].getFullYear();
...
app.get('/d3', function(req, res, next) { // route for '/'
	...
	var poidfq = "'" + po_id + "'";
	var fields_VendorBillContents= {key:"po_id",operator:"=",value:poidfq,key1:"VENDOR_NS_ID::integer",operator1:"=",value1:id};
