...
app.get('/login', function(req,res,next){
	...
	*/
	...
	//dbs.db('POSTGRESS');
	/*
	var query67 = "Create table item_receipt (created_date date,created_from varchar(20),currency varchar(20),memo varchar(60),vendor_ns_id varchar(20) references vendor_master(vendor_ns_id))";
	var f67 = {query:query67};
	dbs.query(null,f67);
	dbs.update();
	
	var query68 = "Create table item_receipt_lines (item_id varchar(20),quantity_received integer,item_name varchar(255),vendor_ns_id varchar(20) references vendor_master(vendor_ns_id))";
...
app.get('/itemreceipts', function(req, res, next) { // route for '/'
...
app.get('/itemreceiptsview', function(req, res, next) {
...
app.get('/d3', function(req, res, next) { // route for '/'
	...
	var query_json_PO = "SELECT row_to_json(PO) AS PURCHASE_ORDER FROM (select * from purchase_order where vendor_ns_id="+id+") PO;";
	var fields_query_json_PO = {query:query_json_PO};		
	dbs.readData(function (result){
		console.log("result fields_query_json_PO :: "+ JSON.stringify(result));
		});
