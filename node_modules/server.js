...
/*	
	...
	var query11 = "DELETE FROM purchase_order WHERE vendor_ns_id = '4219'";
	...
	var query9 = "DELETE FROM po_lines WHERE vendor_ns_id = '4219'";
	var f9 = {query:query9};
	dbs.query(null,f9);
	dbs.update();
	
	var query10 = "Update timestamp set rec_type = 'PurchaseOrder',timestamp='07/10/2016 01:08 AM',local_time='07/10/2016 08:08:10',vendor_ns_id='4219' where rec_type = 'PurchaseOrder' and vendor_ns_id='4219'";
...
app.post('/signup',function(req,res){
	...
	var isperson_val = null;
		...
		else
		{
			...
			if(req.body.con_number)
			{
				console.log("Contact Phone -->" + req.body.con_number);
				//contact_phone_val = req.body.con_number;
			}
...
//var postatus='Pending';
	...
	dbs.readData(function (result1){
		...
		{ 
			...
			time_stamp = result1[0].local_time;
...
//var timestamp_contents = db.exec("select localtime from timestamp where rec_type= 'Bills'");
	...
	if(result != null)
	{
		...
		for (var i=0; i<result.length; i++) 
		{
			...
			date[i] = new Date(result[i].bill_date);
			date_create[i] = weekday[date[i].getDay()] + "  " + month[date[i].getMonth()] + "  " + date[i].getDate() + "  " + date[i].getFullYear();
		...
		*/
		...
		dbs.readData(function (result1){
			...
			if(result1!=null)
			{
				...
				item.item_desc_val = result1[i].description;
...
app.get('/refreshbillpayments', function(req, res, next) {
	...
	//	+ results.billpaymentdetails[i].bill_ns_id + "','"
		if(index<0)
		{
			var insert_query = "insert into bill_payment values('"
						...
						+ results.billpaymentdetails[i].ns_ext_id + "','"
		...
		else
		{
			var update_query= "update bill_payment set ns_id='" + results.billpaymentdetails[i].ns_id
						+ "',ns_ext_id='" + results.billpaymentdetails[i].ns_ext_id
				...
				*/
				...
				if(!is_exist)
				{
						var insert_query = "insert into bill_payment_lines values('"
									...
									+ results.billpaymentdetails[i].itempush[j].orig_amt + "','"
									+ results.billpaymentdetails[i].itempush[j].amt_due + "','"
									+ results.billpaymentdetails[i].itempush[j].date_due + "','"
									+ results.billpaymentdetails[i].itempush[j].payment + "','"
									+ results.billpaymentdetails[i].itempush[j].disc_date + "','"
									+ results.billpaymentdetails[i].itempush[j].disc_avail + "','"
									+ results.billpaymentdetails[i].itempush[j].disc_taken + "','"
									+ id + "','"
				...
				else
				{
					var update_query = "update bill_payment_lines set bill_payment_id='"+results.billpaymentdetails[i].ns_id
								...
								+ "',original_amt='" + results.billpaymentdetails[i].itempush[j].orig_amt
								+ "',amount_due='" + results.billpaymentdetails[i].itempush[j].amt_due
								+ "',date_due='" + results.billpaymentdetails[i].itempush[j].date_due
								+ "',payment='" + results.billpaymentdetails[i].itempush[j].payment
								+ "',disc_date='" + results.billpaymentdetails[i].itempush[j].disc_date
								+ "',disc_avail='" + results.billpaymentdetails[i].itempush[j].disc_avail
								+ "',disc_taken='" + results.billpaymentdetails[i].itempush[j].disc_taken
...
// trigger the restlet
		...
		search.run(rest_params, function (err, results) {
		 ...
		 if(results.status=='Successful')
		 {
		  ...
		  console.log('data_len:::'+data_len);
			...
			console.log('datalength::'+data_len);
			console.log('itemdatalength::'+results.billpaymentdetails[0].itempush.length);
			console.log("tranID : "+results.billpaymentdetails[0].tranID);
			console.log("item_id : "+results.billpaymentdetails[0].itempush[0].item_id);
			console.log("timestamp : "+results.updatetimestamp);
		...
		// insert or update PO and PO lines
			 for(var i=0;i<results.itemreceiptdetails.length;i++)
			{
				...
				var index=item_receipt_list.indexOf(parseInt(results.billpaymentdetails[i].tranID));
				console.log('index::'+index);
				...
				if(index<0)
				{
					var insert_query = "insert into item_receipt values('"
								+ results.billpaymentdetails[i].ns_id + "','"
								+ results.billpaymentdetails[i].tranID + "','"
								+ results.billpaymentdetails[i].createddate + "','"
								+ results.billpaymentdetails[i].po_tranId + "','"
								+ results.billpaymentdetails[i].po_id + "','"
								+ results.billpaymentdetails[i].currency + "','"
								+ results.billpaymentdetails[i].memo + "','"
								+ id + "')";
					var fields_insert_details = {query:insert_query};
				...
				{
					var update_query= "update item_receipt set ns_id='" + results.billpaymentdetails[i].ns_id
								+ "',ns_ext_id='" + results.billpaymentdetails[i].tranID
								+ "',created_date='" + results.billpaymentdetails[i].createddate
								+ "',purchase_order_ext_id='" + results.billpaymentdetails[i].po_tranId
								+ "',purchase_order_id='" + results.billpaymentdetails[i].po_id
								+ "',currency='" + results.billpaymentdetails[i].currency
								+ "',memo='" + results.billpaymentdetails[i].memo
								+ "' where ns_id= '" + results.billpaymentdetails[i].ns_id
								+ "' AND vendor_ns_id='" + id + "'";
					...
					for(var j=0;j<results.billpaymentdetails[i].itempush.length;j++)
					{
						...
						var neg_factor_qty=1;
							if(results.billpaymentdetails[i].itempush[j].quantity_received<1)
							{
								neg_factor_qty=-1;
							}
							/*		var neg_factor_amt=1;
							...
							*/
						console.log('in j loop::'+[results.billpaymentdetails[i].ns_id,results.billpaymentdetails[i].itempush[j].item_id]);
						
						var is_exist=isItemInArray(item_receipt_line_list,[results.billpaymentdetails[i].ns_id,results.billpaymentdetails[i].itempush[j].item_id]);
						...
						if(!is_exist)
						{
							var insert_query = "insert into item_receipt_lines values('"
										+ results.billpaymentdetails[i].ns_id + "','"
										+ results.billpaymentdetails[i].itempush[j].item_id + "','"
										+ results.billpaymentdetails[i].itempush[j].quantity_received + "','"
										+ results.billpaymentdetails[i].itempush[j].item_name + "','"
										+ results.billpaymentdetails[i].itempush[j].vendor_prod_name + "','"
										+ id + "')";
						...
						{
							var update_query = "update item_receipt_lines set item_receipt_id='"+results.billpaymentdetails[i].ns_id
										+ "',item_id='" + results.billpaymentdetails[i].itempush[j].item_id
										+ "',quantity_received='" + results.billpaymentdetails[i].itempush[j].quantity_received
										+ "',item_name='" + results.billpaymentdetails[i].itempush[j].item_name
										+ "',DESCRIPTION='" + results.billpaymentdetails[i].itempush[j].itemName
										+ "',vendor_prod_name='" + results.billpaymentdetails[i].itempush[j].vendor_prod_name
										+ "' where item_receipt_id= '" + results.billpaymentdetails[i].ns_id
										+ "' AND item_id='" + results.billpaymentdetails[i].itempush[j].item_id
										+ "' AND vendor_ns_id='" + id + "'"; 
...
{
	setTimeout(function () {
           res.redirect(303,'/billpaymentsview?ns_id='+item_receipt_ns_id);
        }, 5000);
...
app.get('/billpayments', function(req, res, next) { // route for '/'
