	  ...
	  console.log(JSON.stringify(results));
	   ...
	   console.log('Results login::'+results);
		 ...
		 if(results.status=='Successful')
		{
			...
			var update_query = "update vendor_master set email_id='"+req.body.email+"', phone='"+req.body.phone+"',address='"+req.body.address+"',alt_phone='"+req.body.altphone+"',fax ='"+req.body.fax+"' where vendor_ns_id="+req.body.ns_id;
	...
	if(id)
{
	...
	if(postatus!='none')	
	{
		...
		var fields_timestamp_details= {key:"rec_type",operator:"=",value:rec_type};
		console.log('Before timestamp details :: ');
		dbs.query('timestamp',fields_timestamp_details);
		dbs.readData(function (result){
			console.log("result timestamp details :: "+ JSON.stringify(result));
...
//var timestamp_contents = db.exec("select localtime from timestamp where rec_type= 'Bills'");
	...
	var fields_timestamp_details= {key:"rec_type",operator:"=",value:"'Bills'"};
	console.log('Before timestamp details :: ');
	dbs.query('timestamp',fields_timestamp_details);
	dbs.readData(function (result){
		console.log("result timestamp details :: "+ JSON.stringify(result));
...
if(id)
{
		...
		*/
		...
		var fields_bill_list_ns_details= {key:"BILL_LIST_NS_ID",operator:"=",value:bill_ns_id};
		...
		*/
		...
		var fields_bill_list_line_contents= {key:"BILL_LIST_NS_ID",operator:"=",value:bill_ns_id};
...
if(id)
{
	...
	console.log('values_to_push  ::'+values_to_push);
		...
		dbs.query('packing_list',fields_to_push);
		dbs.value(values_to_push);
...
if(id)
{
	...
	var fields_pl_details= {key:"packing_list_num",operator:"=",value:packinglist_num};
	dbs.query('packing_list',fields_pl_details);
		dbs.readData(function (result){
		console.log("result packing list  details with status all pl :: "+ JSON.stringify(result));
		contents=result;
			console.log("content at all pl :: "+ JSON.stringify(contents));
	...
	for (var i=0; i<contents.length; i++) 
	{
		...
		ship_to_val = contents[i].ship_to;
		ship_to_date_val =contents[i].ship_date;
		delivery_method_val = contents[i].delivery_method;
		shipment_origin_val =contents[i].shipment_origin;
		shipment_point_val = contents[i].shipment_point;	
		memo_val = contents[i].Memo;				
			
	} 
	});
	
	var fields_pllines_details= {key:"packinglist_ref",operator:"=",value:packinglist_num};
	dbs.query('packing_list_lines',fields_pllines_details);
		dbs.readData(function (result){
		console.log("result packing list  details with status all pl :: "+ JSON.stringify(result));
	...
	if(pl_line_contents!='' || pl_line_contents!=null)
	{
		console.log("po_line_contents length : "+pl_line_contents.length);
		...
		for (var i=0; i<pl_line_contents.length; i++) 
		{
			...
			item.item_id_val = pl_line_contents[i].item_id;
			item.item_name_val = pl_line_contents[i].item_name; 
			item.description_val = pl_line_contents[i].description;
			item.qty_ordered_val = pl_line_contents[i].qty_ordered;
			item.qty_dispatched_val = pl_line_contents[i].qty_dispatched;
			item.netwt_val = pl_line_contents[i].net_wt;
			item.grosswt_val =pl_line_contents[i].gross_wt;
			item.po_ref =pl_line_contents[i].REF_PO_NUM;
