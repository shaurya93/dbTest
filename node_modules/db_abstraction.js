...
var dba = dba || {};
	...
	module.exports.readVendor = function(callback){
		...
		client.connect();	
				...
				if(this.fields == null)
				{
					var rows= client.query("select * from "+this.table , function(err, result) {
    					console.log('Query_result before return :: ' + JSON.stringify(rows));
					client.end();
					callback (rows);
					});
				}
				...
				{
					var rows = client.query("select * from " + this.table + " where " + this.fields.key 
					+ this.fields.operator + this.fields.value,function(err,result){

					console.log('Query_result before return when fields not null :: ' + JSON.stringify(rows));
					client.end();
					callback (rows);
					});
